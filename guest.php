...
<div class="page">
  ...
  <div class="stats">
    <div class="stat-card pink">
      <div class="stat-label">จำนวนแขก</div>
      <div class="stat-value"><?= number_format($total_guests) ?> <small style="font-size:18px;color:var(--text-mid)">คน</small></div>
    </div>
    <div class="stat-card gold">
      <div class="stat-label">ยอดรวมเงินซอง</div>
    ...
    <div class="stat-card gold">
      ...
      <div class="gold-text stat-value"><?= number_format($total_cost) ?> <small style="font-size:18px">฿</small></div>
    </div>
    <div class="stat-card gold">
      <div class="stat-label">ยอดรวมเงินซอง - ค่าใช้จ่าย</div>
      <div class="gold-text stat-value">  <?= number_format($total_amount - $total_cost) ?>
      <small style="font-size:18px">฿</small></div>
    </div>
  </div>
  ...
  <div class="toolbar">
    <form method="GET" class="search-wrap">
      ...
      <input type="text" name="q" placeholder="ค้นหาชื่อแขก..." value="<?= htmlspecialchars($search) ?>">
    </form>
  </div>
      ...
      <tbody>
        ...
        <tr><td colspan="5">
          <div class="empty">
            ...
            <h3><?= $search ? 'ไม่พบแขกที่ค้นหา' : 'ยังไม่มีรายชื่อแขก' ?></h3>
            <p><?= $search ? 'ลองค้นหาด้วยคำอื่น' : 'กดปุ่ม "เพิ่มแขก" เพื่อเริ่มต้น' ?></p>
          </div>
        </td></tr>
        <?php else: ?>
        ...
        <tr>
          ...
          <td class="text-center">
            <div class="actions">
              <button class="act-btn edit" title="แก้ไข"
                onclick="openEdit(<?= $g['id'] ?>, '<?= addslashes(htmlspecialchars($g['guest_name'])) ?>', <?= $g['money'] ?>, '<?= addslashes(htmlspecialchars($g['remark'] ?? '')) ?>',<?= $g['guest_type'] ?>)">✏️</button>
              <form method="POST" onsubmit="return confirm('ลบ <?= addslashes(htmlspecialchars($g['guest_name'])) ?> ใช่ไหม?')" style="display:inline">
          ...
          <td class="guest-name"><?= $g['guest_name'] ?></td>
          <td><span class="amount-badge">฿<?= $g['money'] ?></span></td>
          <td class="guest-name"><?= $g['guest_type_name'] ?></td>
...
<div class="overlay" id="overlay" onclick="closeOnBg(event)">
  <div class="modal">
    ...
    <div class="modal-title" id="modal-title">เพิ่มข้อมูล</div>

    <form method="POST" id="modal-form" action="/api/guestMoney/save.php">
      ...
      <div class="form-field">
        ...
        <div class="form-field">
            <label>ฝั่ง *</label>
            <select name="guestType" id="form-side">
                <option value="0">ญาติฝั่งเจ้าบ่าว</option>
                <option value="1">ญาติฝั่งเจ้าสาว</option>
            </select>
        </div>

      <div class="form-field">
        <label>หมายเหตุ</label>
        <textarea name="remark" id="form-note" placeholder="เช่น เพื่อนเจ้าบ่าว, ญาติฝั่งเจ้าสาว"></textarea>
      </div>
...
<script>
  ...
  function openEdit(id, name, amount, note,guestType) {
    ...
    document.getElementById('form-side').value = guestType;
    document.getElementById('save-btn').textContent = '✦ บันทึกข้อมูล';
    document.getElementById('overlay').classList.add('show');
    setTimeout(() => document.getElementById('form-name').focus(), 200);
  }
